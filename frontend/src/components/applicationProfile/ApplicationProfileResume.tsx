...
const ApplicationProfileResume: React.FC<IApplicationProfileResumeProps> = ({resumeUrl}) => {
    const {t} = useTranslation();

    const downloadResume = () => {
        window.location.href = `${baseUrl}/${resumeUrl}`;
    };

    return (
        <div className="resume">
            <h3 className="resume__title">Резюме</h3>
            <div className="resume__content">
                <div className="resume__name">
                    ...
                    />
                    <span className="resume__name-file">Резюме</span>
                </div>
                <button className="resume__download" onClick={downloadResume}>{t("resume_download")}</button>
            </div>
        </div>
    );
};
