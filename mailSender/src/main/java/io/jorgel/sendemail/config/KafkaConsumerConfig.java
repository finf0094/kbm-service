...
//                + KAFKA_USER + "' password='" + KAFKA_PASSWORD + "';");
    ...
    public ConcurrentKafkaListenerContainerFactory<String, String> kafkaListenerContainerFactory() {
        ...
        factory.setErrorHandler(((thrownException, data) -> LOGGER.log(Level.SEVERE, () -> "Kafka -> Exception in consumerConfig is: " + thrownException.getMessage() + " data: " + data)));
        factory.setMessageConverter(new StringJsonMessageConverter());
        factory.setRetryTemplate(retryTemplate());
        factory.setRecoveryCallback(retryContext -> {
            if (retryContext != null) {
                LOGGER.log(Level.SEVERE, () -> "kafkaListenerContainerFactory Error -> " + (retryContext.getLastThrowable() != null ? retryContext.getLastThrowable().getCause() : "unknown"));
                ...
                Arrays.asList(retryContext.attributeNames())
                        .forEach(attributeName -> {
                            ...
                            Object attributeValue = retryContext.getAttribute(attributeName);
                            LOGGER.log(Level.INFO, () -> "Attribute Value is: " + (attributeValue != null ? attributeValue.toString() : "null"));
                        });
                if (retryContext.hasAttribute("record")) {
                    ConsumerRecord<?, ?> consumerRecord = (ConsumerRecord<?, ?>) retryContext.getAttribute("record");
                    if (consumerRecord != null) {
                        String errorTopic = consumerRecord.topic().replace("-retry", "-error");
